...
namespace fmiVis {
	...
	//make a viewport from x1y1 to x2y2 in screenspace
	...
	//uses a non-flat surfacenbs to make a heightmesh that is then skinned with a map image
	void AddMapPlane(vtkSmartPointer<vtkRenderer> ren, const std::string &file, nbsMetadata &meta, bool flat) {
		...
		std::map<int, std::string> &paramsSurf, NFmiFastQueryInfo &dataInfo)
	{
		...
		//smartmet options are used on some visualizers
	...
	void ViewportFactory::MakeTimeGridView(size_t numX, size_t numY,
		std::string file, std::string drawParamPath, nbsMetadata &meta, NFmiFastQueryInfo &dataInfo, ViewportManagerTimegrid &viewportMan,
		vtkRenderWindowInteractor *iren, vtkRenderWindow *renWin, VisualizationInteractor2D *style)
	{
		//The renderer uses the information provided by actors to render the data their mappers prepare
		...
		for (size_t y = 0; y < numY; y++)
			for (size_t x = 0; x < numX; x++)
			{
				...
				//visualizerman controls the visualizers, relaying UI commands to them and updating them
				...
				//the visualizers are charged with querying a newbasesourcer for the data and mapping it to be drawn by the renderer
		...
		//The timeanimator contains the logic for advancing timesteps manually or through playback (space)
