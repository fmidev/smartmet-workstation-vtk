...
namespace fmiVis {
		...
		newBaseSourcerBase(file, meta, param, 1), flat(flat), pad(pad), spacing(2)
	{
		...
		//the (possibly heightmapped) mesh
	...
	//provide the VTK pipeline with information about available data
	...
	bool nbsSurface::loadPoints() {
		...
		//a simple grid is generated
		//if you can assign point data to some plane generated by VTK, it would not have to be triangulated
		if (flat) {
			...
			for (int x = 0; x < gridX; ++x)
				for (int y = 0; y < gridY; ++y)
				{
					points->InsertNextPoint(x * spacing - (pad ? spacing : 0),
						...
						values[x + y * sizeX] * spacing / zHeight * 80); //TODO zHeight from VisualizerManager and make newBaseSourcer zRes visible
		...
		//an expensive process for large grids
	...
	int nbsSurface::RequestData(vtkInformation* vtkNotUsed(request),
		...
		//don't read a timestep we already have
		...
		//report the actual timestep found
